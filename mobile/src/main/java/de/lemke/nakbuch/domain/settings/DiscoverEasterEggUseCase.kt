...
class DiscoverEasterEggUseCase {
    operator fun invoke(mContext: Context, konfettiView: KonfettiView, easterEggEntryNameResourceId: Int) =
        invoke(mContext, konfettiView, mContext.getString(easterEggEntryNameResourceId))

    operator fun invoke(mContext: Context, konfettiView: KonfettiView, easterEggEntryName: String) {
        if (AreEasterEggsEnabledUseCase()()) {
            if (settingsRepo.discoverEasterEgg(easterEggEntryName)) {
                showKonfetti(konfettiView)
                Toast.makeText(mContext, mContext.getString(R.string.easterEggDiscovered) + easterEggEntryName, Toast.LENGTH_SHORT)
                    .show()
            }
